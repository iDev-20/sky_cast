...
import 'package:sky_cast/services/time_view_model.dart';
import 'package:sky_cast/utilis/constants.dart';
import 'package:sky_cast/utilis/navigation.dart';
import 'package:sky_cast/widgets/city_time_card.dart';
...
class _TimePageState extends State<TimePage> {
  ...
  List<CityTimeCard> cityTimeCard = [];
  ...
  Future<void> searchTime() async {
    ...
    try {
      ...
      if (time != null) {
        final alreadyExists = cityTimeCard.any((card) =>
            card.timeZone.toLowerCase() == time.timeZone.toLowerCase());
  ...
  Future<void> refreshTime() async {
    ...
    try {
      ...
      if (time != null) {
        final index = cityTimeCard.indexWhere((card) =>
            card.timeZone.toLowerCase() == time.timeZone.toLowerCase());

        setState(() {
          if (index != -1) {
            cityTimeCard[index] = CityTimeCard(
              time: time.time,
              date: time.date,
              timeZone: time.timeZone,
              day: time.dayOfWeek,
                      ...
                      child: Padding(
                        ...
                        child: RefreshIndicator(
                          ...
                          child: ListView.builder(
                              ...
                              shrinkWrap: true,
                              itemCount: cityTimeCard.length,
                              itemBuilder: (context, index) {
                                return cityTimeCard[index];
                              }),
                        ),
                      ),
                    ),
